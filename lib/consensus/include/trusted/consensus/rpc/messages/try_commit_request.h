...
#ifndef LIB_TRUSTED_CONSENSUS_RPC_MESSAGE_TRY_COMMIT_REQUEST_H
#define LIB_TRUSTED_CONSENSUS_RPC_MESSAGE_TRY_COMMIT_REQUEST_H
...
namespace Consensus {
    class TryCommitRequestMessage : public MessageBase {
    private:
        int nodeId;
        int logId;
        size_t size;
    ...
    public:
        TryCommitRequestMessage() : MessageBase(TryCommitRequest), nodeId(-1), logId(-1), size(0), buffer() {}

        explicit TryCommitRequestMessage(int nodeId, int logId, size_t size, const std::shared_ptr<char> &buffer)
                : MessageBase(TryCommitRequest), nodeId(nodeId), logId(logId), size(size), buffer(buffer) {}
...
#endif //LIB_TRUSTED_CONSENSUS_RPC_MESSAGE_TRY_COMMIT_REQUEST_H
