...
#ifndef LIB_TRUSTED_CONSENSUS_RPC_MESSAGE_TRY_ACCEPT_REQUEST_H
#define LIB_TRUSTED_CONSENSUS_RPC_MESSAGE_TRY_ACCEPT_REQUEST_H
...
namespace Consensus {
    class TryAcceptRequestMessage : public MessageBase {
    private:
        int nodeId;
        int logId;
        size_t size;
    ...
    public:
        TryAcceptRequestMessage() : MessageBase(TryAcceptRequest), nodeId(-1), logId(-1), size(0), buffer() {}

        explicit TryAcceptRequestMessage(int nodeId, int logId, size_t size, const std::shared_ptr<char> &buffer)
                : MessageBase(TryAcceptRequest), nodeId(nodeId), logId(logId), size(size), buffer(buffer) {}
...
#endif //LIB_TRUSTED_CONSENSUS_RPC_MESSAGE_TRY_ACCEPT_REQUEST_H
