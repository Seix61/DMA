...
#ifndef LIB_TRUSTED_CONSENSUS_RPC_MESSAGE_ACCEPT_REQUEST_H
#define LIB_TRUSTED_CONSENSUS_RPC_MESSAGE_ACCEPT_REQUEST_H
...
namespace Consensus {
    class AcceptRequestMessage : public MessageBase {
    private:
        int logId;
        size_t size;
        std::shared_ptr<char> buffer;
    public:
        AcceptRequestMessage() : MessageBase(AcceptRequest), logId(-1), size(0), buffer() {}

        explicit AcceptRequestMessage(int logId, size_t size, const std::shared_ptr<char> &buffer)
                : MessageBase(AcceptRequest), logId(logId), size(size), buffer(buffer) {}
...
#endif //LIB_TRUSTED_CONSENSUS_RPC_MESSAGE_ACCEPT_REQUEST_H
