...
EpidServer::EpidServer(int port) : SSLServer(port) {}

int EpidServer::setCertCallback(SSL *session, void *arg) {
    size_t c_size;
...
int EpidServer::verifyCallback(int pre_verify_ok, X509_STORE_CTX *ctx) {
    ...
    return verifyCert(crt, s_nonce.get(), s_size, c_nonce.get(), c_size, SSL_get_fd(ssl));
}

sgx_status_t EpidServer::create() {
    do {
