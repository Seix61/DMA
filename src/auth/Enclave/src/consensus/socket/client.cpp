...
ConsensusClient::ConsensusClient(const char *serverName, int port) : SSLClient(serverName, port) {}

int ConsensusClient::setCertCallback(SSL *session, void *arg) {
    return 1;
    size_t s_size;
...
int ConsensusClient::verifyCallback(int pre_verify_ok, X509_STORE_CTX *ctx) {
    ...
    return verifyCert(crt, c_nonce.get(), c_size, s_nonce.get(), s_size);
}

sgx_status_t ConsensusClient::create() {
    do {
