...
UserServer::UserServer(int port) : SSLServer(port) {}

int UserServer::setCertCallback(SSL *session, void *arg) {
    size_t c_size;
...
int UserServer::verifyCallback(int pre_verify_ok, X509_STORE_CTX *ctx) {
    return 1;
...
sgx_status_t UserServer::create() {
    do {
