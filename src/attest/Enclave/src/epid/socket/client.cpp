...
EpidClient::EpidClient(const char *serverName, int port) : SSLClient(serverName, port) {}

int EpidClient::setCertCallback(SSL *session, void *arg) {
    size_t s_size;
...
int EpidClient::verifyCallback(int pre_verify_ok, X509_STORE_CTX *ctx) {
...
sgx_status_t EpidClient::create() {
    do {
